...
#     else:
#         results = hybrid_search(query, vector_store, bm25, embed_texts)
#         results = simple_rerank(query, results)

#         # 🔹 KG enrichment (Onyx-style)
...
#     prompt = f"""
# Convert this question into a SHORT keyword search query.
...
#         rewritten = " ".join(rewritten.split()[:8])  # limit length
...
#         context_chunks = web_search(query)
#     else:
#         results = hybrid_search(search_query, vector_store, bm25, embed_texts)
#         results = simple_rerank(query, results)
#         context_chunks = enrich_with_kg(results)
